	...
	<div>
		...
		<div class="main">
			<dl>
				...
				<dd class="d1"><img :src="this.imgSrc" alt="" width="100%"></dd>
				<dd class="d2">{{brand.Title}}<i class="rili"></i><i class="shijian">{{createTime}}</i></dd>
				<dd class="buildContent"  v-html="brand.Content">{{brand.Content}}</dd>
			</dl>
		</div>
		<div class="foot"  >
...
<style lang="scss" scoped>
	...
	.main{
		...
		dl{
			...
			.d1{
				@include wh(100%,8.7rem);
				padding-bottom:1.2rem;
			}
			.d2{
				@include sc(0.95rem,#565656);
				font-weight:bold;
...
<script>
	import {queryBrand} from '../../config/data-service'
	export default{
		data(){
			return{
		...
		mounted(){
			this.bdId = this.$route.query.brandId;
			queryBrand(this.bdId).then(res =>{
				console.log(res);
				this.brand = res;
				this.imgSrc = 'https://d.carbros.cn:4433/GplusServer/Site/QueryImg?FileName='+this.brand.BannerImg+'&FileType=News'
				this.createTime = this.brand.CreateTime.substring(0,this.brand.CreateTime.indexOf('T'));
			})
		}
	}
</script>